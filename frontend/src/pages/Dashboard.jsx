      ...
      <div className="mx-auto max-w-7xl space-y-8">
        ...
        <section className="rounded-xl bg-white p-6 shadow-sm border border-slate-200 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <button
            ...
            className="inline-flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 active:bg-blue-800 text-white font-semibold text-sm px-6 py-3 rounded-lg shadow-sm hover:shadow transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:ring-offset-2 whitespace-nowrap"
          >
            <FilePlus size={18} />
            Apply for Service
            <ArrowRight size={16} />
        ...
        <section className="rounded-xl bg-white p-6 shadow-sm border border-slate-200">
          ...
          {applications.length === 0 ? (
            <div className="text-center py-12">
              ...
              <button onClick={() => navigate("/applyService")} className="text-sm font-semibold text-blue-600 border border-blue-200 px-4 py-2 rounded-lg hover:bg-blue-50">Create First Application</button>
            </div>
          ) : (
                ...
                <tbody className="divide-y divide-slate-100">
                  {applications.map((application) => (
                    <tr key={application.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="p-4">{application.service_type?.replaceAll("_", " ") || "General Service"}</td>
                      <td className="p-4 font-mono text-xs">{application.county_id || "N/A"}</td>
